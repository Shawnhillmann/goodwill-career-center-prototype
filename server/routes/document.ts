...
function resumeFileBase(content: string, fallback: string): string {

  const parsed = parseResumeDocument(content)

  if (parsed?.name) return safeFileBase(parsed.name)

  return safeFileBase(fallback)
...
async function sendDocx(res: express.Response, opts: { fileBase: string; markdown: string }) {
  ...
  const resume = parseResumeDocument(opts.markdown)
...
async function sendPdf(res: express.Response, opts: { fileBase: string; markdown: string }) {
  ...
  if (resume) {
    ...
    const doc = streamResumePdf(resume)
...
documentRouter.post('/export', async (req, res) => {
  ...
  const fallback = body.format === 'pdf' ? 'document' : 'resume'

  const fileBase = resumeFileBase(body.content, body.fileName ?? fallback)
...
documentRouter.post('/resume', async (req, res) => {
  ...
  const fileBase = resumeFileBase(body.resumeText, body.fileName ?? 'resume')
