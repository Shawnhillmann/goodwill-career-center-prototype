...
export function LiveSupportPage({ language, onBack }: LiveSupportPageProps) {
  ...
  return (
    <section className="support" aria-label={ ui.liveSupportTitle }>
      ...
      <section className="support-booking" aria-labelledby="support-booking-heading">
        <h2 id="support-booking-heading" className="support__section-title">
          { ui.liveSupportBookingHeading }
        </h2>
        <p className="support-booking__subtitle">{ ui.liveSupportBookingSubtitle }</p>
        <iframe
          className="support-booking__iframe"
          src="https://www.picktime.com/GCCvirtual#book/services"
          title={ ui.liveSupportBookingIframeTitle }
          loading="lazy"
          ...
          {sortedCenters.map((center, index) => (
            <li key={ center.id } className="support-center">
              ...
              <p className="support-center__row">
                <MapPin size={ 16 } fill="currentColor" strokeWidth={ 0 } aria-hidden />
                <span>{ formatAddress(center) }</span>
              </p>
              <p className="support-center__row">
                <Phone size={ 16 } fill="currentColor" strokeWidth={ 0 } aria-hidden />
